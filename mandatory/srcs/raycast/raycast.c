...
/*   Updated: 2025/10/26 14:36:44 by nofanizz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calc_tex_x(t_tex *tex, t_wall *wall, t_ray *ray, t_dda *dda)
{
	...
	if (dda->side == 0 && ray->cos_angle < 0)
		tex->tex_x = tex->tex->width - tex->tex_x - 1;
	if (dda->side == 1 && ray->sin_angle > 0)
		tex->tex_x = tex->tex->width - tex->tex_x - 1;
	if (tex->tex_x < 0)
		tex->tex_x = 0;
                 ...
                 double player_grid_x, double player_grid_y)
{
    if (dda->side == 0)
    {
        wall->wall_x = player_grid_y;
        wall->wall_x += (ray->map_x - player_grid_x
            + (1 - dda->step_x) * 0.5) / ray->cos_angle * ray->sin_angle;
    }
    else
    {
        wall->wall_x = player_grid_x;
        wall->wall_x += (ray->map_y - player_grid_y
            + (1 - dda->step_y) * 0.5) / ray->sin_angle * ray->cos_angle;
    }
...
void init_ray(t_ray *ray, t_data *data, double ray_angle, double inv_tile_size)
{
    ...
    ray->map_x = (int)(data->player.pos.x * inv_tile_size);
    ray->map_y = (int)(data->player.pos.y * inv_tile_size);
}
